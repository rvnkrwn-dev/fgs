  ...
  <div class="flex flex-1 flex-col gap-6 p-6 pt-0">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-muted-foreground mt-1">Kelola semua registrasi peserta Festival Gunung Selamet</p>
      </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
      <Card class="border-0 shadow-lg bg-gradient-to-br from-blue-50 to-blue-100">
        <CardContent class="p-4">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm font-medium text-blue-700">Total Registrasi</p>
      ...
      <Card class="border-0 shadow-lg bg-gradient-to-br from-green-50 to-green-100">
        <CardContent class="p-4">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm font-medium text-green-700">Event Aktif</p>
              <p class="text-2xl font-bold text-green-900">{{ getUniqueEventsCount() }}</p>
            </div>
      ...
      <Card class="border-0 shadow-lg bg-gradient-to-br from-orange-50 to-orange-100">
        <CardContent class="p-4">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm font-medium text-orange-700">Hari Ini</p>
              <p class="text-2xl font-bold text-orange-900">{{ getTodayRegistrations() }}</p>
            </div>
      ...
      <CardContent>
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
          <div class="space-y-2">
            ...
            <div class="relative">
              ...
              <Input
                  ...
                  placeholder="Nama, WhatsApp, Email..."
                  class="pl-10"
          ...
          <div class="space-y-2">
            ...
            <Select v-model="filters.eventId" @update:model-value="fetchRegistrations">
              <SelectTrigger>
                <SelectValue placeholder="Semua Event" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="ALL">Semua Event</SelectItem>
                <SelectItem v-for="event in uniqueEvents" :key="event.id" :value="event.id">
                  {{ event.title }}
                </SelectItem>
              </SelectContent>
            </Select>
        ...
        <div class="flex justify-between items-center mt-4">
          <div class="text-sm text-muted-foreground">
            Menampilkan {{ registrations.length }} dari {{ pagination.total }} registrasi
          </div>
    ...
    <Card v-else class="border-0 shadow-lg overflow-hidden">
      <CardContent class="p-0">
          ...
          <Table>
            <TableHeader>
              <TableRow class="bg-gray-50">
                ...
                <TableHead class="font-semibold">Peserta</TableHead>
            ...
            <TableBody>
              <TableRow v-for="registration in registrations" :key="registration.id" class="hover:bg-green-50/50 transition-colors">
                ...
                <TableCell>
                  <div class="space-y-1">
                    <p class="text-sm font-medium">{{ registration.event.title }}</p>
        ...
        <div v-if="!loading && registrations.length === 0" class="text-center py-16">
          ...
          <h3 class="text-xl font-semibold text-foreground mb-2">Belum ada registrasi</h3>
          <p class="text-muted-foreground mb-6 max-w-md mx-auto">
            Belum ada peserta yang mendaftar untuk Festival Gunung Selamet. Data registrasi akan muncul di sini.
          </p>
        </div>
      </CardContent>
    </Card>
    ...
    <Card v-if="selectedRegistrations.length > 0" class="border-0 shadow-lg bg-gradient-to-r from-blue-50 to-green-50">
      <CardContent class="p-4">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <p class="text-sm font-medium text-foreground">
              {{ selectedRegistrations.length }} registrasi dipilih
            </p>
          </div>
        ...
        <div v-if="selectedRegistration" class="space-y-6">
          ...
          <Card>
            <CardContent class="p-4">
              ...
              <div class="space-y-2 text-sm">
                <p><strong>Nama Event:</strong> {{ selectedRegistration.event.title }}</p>
                <p><strong>Tanggal Event:</strong> {{ formatDate(selectedRegistration.event.start_date) }}</p>
                <p><strong>Lokasi:</strong> {{ selectedRegistration.event.location }}</p>
              </div>
            </CardContent>
          </Card>
        </div>
...
const uniqueEvents = computed(() => {
  const events = registrations.value.map(r => r.event)
...
const getUniqueEventsCount = () => {
  return new Set(registrations.value.map(r => r.event_id)).size
}
...
const fetchRegistrations = async () => {
  ...
  try {
    ...
    if (response.statusCode === 200) {
      ...
      error.value = response.message || 'Gagal mengambil data registrasi'
    }
  } catch (err: any) {
    console.error('Error fetching registrations:', err)
    error.value = err.data?.message || 'Terjadi kesalahan saat mengambil data registrasi'
  } finally {
    loading.value = false
  }
}
...
const contactParticipant = (registration: Registration) => {
  const message = `Halo ${registration.full_name}, terima kasih telah mendaftar untuk ${registration.event.title}!`
...
const exportData = () => {
  const csvContent = [
    ['Nama', 'WhatsApp', 'Event', 'Provinsi', 'Kota', 'Kecamatan', 'Kode Pos', 'Tanggal Daftar'],
  ...
  const a = document.createElement('a')
  a.href = url
  a.download = `registrasi_festival_${new Date().toISOString().split('T')[0]}.csv`
  a.click()
  URL.revokeObjectURL(url)
}
...
const bulkExport = () => {
  ...
  const a = document.createElement('a')
  a.href = url
  a.download = `registrasi_terpilih_${new Date().toISOString().split('T')[0]}.csv`
  a.click()
  URL.revokeObjectURL(url)
}
...
const bulkMessage = () => {
  ...
  const message = `Halo! Terima kasih telah mendaftar untuk Festival Gunung Selamet. Kami akan segera menghubungi Anda dengan informasi lebih lanjut.`

  // Open WhatsApp Business API or show phone numbers
